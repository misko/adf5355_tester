...
def cmd_off(args) -> int:
    ...
    This is the command reached for when something else has already gone
    wrong, so it is built to have the fewest dependencies of anything in the
    CLI. In particular it does NOT claim CE or MUXOUT: turning the outputs off
    needs neither, and a crashed transmitter still holding those lines was
    observed to make the old implementation fail with 'GPIO busy' -- an off
    switch that stops working precisely when a transmitter is stuck is the
    wrong shape. Pass --ce-gpio/--muxout-gpio explicitly if you want them.
    """
    config = build_config(args, False, False)
    dev = ADF5355(
        config, bus=args.bus, device=args.device, spi_hz=args.spi_hz,
        ce_gpio=None, muxout_gpio=None, dry_run=args.dry_run,
    ).open()
    try:
        r6, r4 = dev.force_off(plan(config, args.freq, resolve_channel(args)))
        print(f"outputs disabled  R6 0x{r6:08X}  R4 0x{r4:08X} (power_down)")
        print("the part cannot be read back, so this reports what was written, "
              "not a measurement that the output is silent")
    finally:
        dev.close()
    return 0
