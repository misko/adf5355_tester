...
class ADF5355:
    ...
    def force_off(self, p: Plan) -> tuple[int, int]:
        """Disable both outputs and power the synthesiser down. Returns (R6, R4).

        Deliberately narrower than program()+mute(). This is the command
        reached for when something else has already gone wrong, so it depends
        on as little as possible: it needs no prior plan in this process, and
        it writes two registers rather than all thirteen -- a full program
        walks the part through a keyed state on its way to disabling it, which
        is the opposite of what an off command should do.

        Order matters. R6 stops the RF while the loop is still running
        normally; R4 then powers the loop down. Reversed, the loop dies with
        the output stage still keyed.

        The caller is expected to have opened the device without claiming GPIO
        -- turning the outputs off needs neither CE control nor lock detect,
        and a crashed transmitter still holding those lines must not be able to
        prevent a shutdown. tools/adf_off.c is the same two writes with no
        Python at all, for when even this is too much machinery.
        """
        r6 = p.words[6]
        r4 = p.words[4] | _POWER_DOWN.encode(1)
        self._write(6, r6)
        self._write(4, r4)
        self._muted = True
        return r6, r4
